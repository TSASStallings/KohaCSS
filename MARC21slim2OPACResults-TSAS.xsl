...
<xsl:stylesheet version="1.0"
  ...
  exclude-result-prefixes="marc items str" extension-element-prefixes="exsl">
    ...
    <!-- TSAS: Helper template to strip trailing punctuation (/, ., :, space) -->
    <xsl:template name="stripTrailingPunctuation">
        <xsl:param name="text"/>
        <xsl:variable name="trimmed" select="normalize-space($text)"/>
        <xsl:variable name="lastChar" select="substring($trimmed, string-length($trimmed))"/>
        <xsl:choose>
            <xsl:when test="$lastChar = '/' or $lastChar = '.' or $lastChar = ':' or $lastChar = ';'">
                <xsl:call-template name="stripTrailingPunctuation">
                    <xsl:with-param name="text" select="substring($trimmed, 1, string-length($trimmed) - 1)"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
...
-->
    ...
    <a>
        ...
        <!-- TSAS: Simplified title output with punctuation stripping -->
        <xsl:if test="marc:datafield[@tag=245]">
            <xsl:for-each select="marc:datafield[@tag=245]">
                <!-- 245$a - Main title, strip trailing punctuation -->
                <xsl:call-template name="stripTrailingPunctuation">
                    <xsl:with-param name="text" select="marc:subfield[@code='a']"/>
                </xsl:call-template>
                <!-- 245$b - Subtitle, if present -->
